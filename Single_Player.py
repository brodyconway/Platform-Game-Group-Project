...
def serve_single_games():
    ...
    Single_Player_background = pygame.image.load(os.path.abspath(
        "ImageResource/Single Player Selection Screen.png"))
    Single_Player_background_bright = GlobalVar.apply_brightness(Single_Player_background, LocalBrightness * 0.5)
    Single_Player_Label = pygame.image.load(
        os.path.abspath("ImageResource/CharacterSelection/Label of Single Player.png"))
    Single_Player_Label_bright = GlobalVar.apply_brightness(Single_Player_Label, LocalBrightness * 0.5)
    # Initialize the grid
    create_grid()
    selection_count = 0
    # Main game loop
    ...
    while running and (GlobalVar.STATUS=="Single Selection" or GlobalVar.STATUS=="AI Mode"):
        screen.fill(WHITE)
        ...
        Label_image = pygame.transform.scale(Single_Player_Label_bright, (Image_width*0.5, Image_height*0.1))
        # Update the display
        screen.blit(regulate_image, (0, 0))
        screen.blit(Label_image, (Image_width*0.25, Image_height*0.15))
        x = 0
